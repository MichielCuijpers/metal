...
public class ParseGraphTest {
    ...
    private static ParseValue makeValWithDef(final char name, final Token token, final long offset) {
        return new ParseValue(Character.toString(name), token, ImmutableList.create(new Location(offset, 1)), new byte[] { (byte) name }, enc());
    }
    ...
    private ParseGraph makeCycleGraph() {
        return ParseGraph
            ...
            .addBranch(t)
            .add(b)
            .add(new ParseReference(a.location.head.offset, aDef))
            .closeBranch();
    }
